function  OutSchr = Schrodinger3(OutGenerate,Ec_new,Ev_new)
...
%     Eigen_val_C = diag(Eigen_val_matrix);  % in Rydbergs
    ...
    HHC=sparse(Hc);
    options.disp=0;
    [Psi_C(2:end-1,1:N_eig) Eigen_val_matrix]=eigs(HHC,N_eig,'sm',options);
    for i=1:size(Psi_C(2:end-1,:),2)
        Psi2_C(2:end-1,i) = ( conj(Psi_C(2:end-1,i)./Ldiag') ).* (Psi_C(2:end-1,i)./Ldiag');
    end
...
%     plot(Psi_C(:,10));hold all; plot(Psi_C1(:,10));
%     
%     reply=input('Hit Enter','s');
...
%      [Psi_V(2:end-1,:) Eigen_val_matrix] = eig(Hv,'nobalance');
%     
...
%     Eigen_val_V = -diag(Eigen_val_matrix);  % in Rydbergs
    ...
    HHV=sparse(Hv);
    options.disp=0;
    [Psi_V(2:end-1,1:N_eig) Eigen_val_matrix] = eigs(HHV,N_eig,'sm',options);
