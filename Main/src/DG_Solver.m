function [VDG , n_charge_DG , p_charge_DG] = DG_Solver(fitting_factor,OutGenerate,Vbetterguess,A,Vt,Q)
  ...
  % Initial Charge Density Calculation
    % Charge Density Calculation             
        [~, n_charge_DG, p_charge_DG, ~] = charge_density(Vbetterguess,OutGenerate,(Ec-Q*Vbetterguess),(Ev-Q*Vbetterguess),0,1);
...
%         n_charge_DG = zeros(No_of_nodes-2,1);
	    ...
	    [ n_charge_DG ] = Newton_Raphson_Solver( fitting_factor,No_of_nodes,x,n_charge_DG,Vt,Q,Vbetterguess,ni,n0,p0,-1 );

	    % Solving the Density Gradient Equation for hole density
	    [ p_charge_DG ] = Newton_Raphson_Solver( fitting_factor,No_of_nodes,x,p_charge_DG,Vt,Q,Vbetterguess,ni,n0,p0,1 );

	    % Construcion of b matrix of poisson equation
        for counter1=1:No_of_nodes-2
	        if counter1==1
	             ffinal(counter1,1)=(((x(2)-x(1))^2)/epsilon0(counter1+1))*Q*((p_charge_DG(counter1+1))-(n_charge_DG(counter1+1))+Nd(counter1+1)-Na(counter1+1))+ Vbetterguess(1);
	        elseif counter1==(No_of_nodes-2)
	             ffinal(counter1,1)=(((x(No_of_nodes-1)-x(No_of_nodes-2))^2)/epsilon0(counter1+1))*Q*((p_charge_DG(counter1+1))-(n_charge_DG(counter1+1))+Nd(counter1+1)-Na(counter1+1)) + Vbetterguess(No_of_nodes);
	        else
	             ffinal(counter1,1)=(((x(counter1+1)-x(counter1))^2)/epsilon0(counter1+1))*Q*((p_charge_DG(counter1+1))-(n_charge_DG(counter1+1))+Nd(counter1+1)-Na(counter1+1));
	        end
        end
