function [ charge ] = Newton_Raphson_Solver( fitting_factor,No_of_nodes,x,charge,Vt,Q,Vbetterguess,ni,n0,p0,indicator )
% Newton Raphson Solver for electron and hole density
  ...
  % Values of Constants
    bn  = fitting_factor/(4*1.08*0.5*Q);            %bn calculation
    bp  = fitting_factor/(4*0.57*0.5*Q);            %bp calculation
...
%       disp(iteration);         
      ...
      for counter1=1:No_of_nodes-2
          if counter1==1
              fn(counter1,1)=((b/((x(1,2)-x(1,1))^2))*((sqrtdensity(counter1+1,1))-2*(sqrtdensity(counter1,1))+(init)))+(((sqrtdensity(counter1,1))/2)*(Vbetterguess(1,counter1+1)+((indicator)*((1/Q)*Vt*log((sqrtdensity(counter1,1)^2)/ni(1,counter1+1))))-phi));
          elseif counter1==(No_of_nodes-2)
              fn(counter1,1)=((b/((x(1,No_of_nodes-1)-x(1,No_of_nodes-2))^2))*((last)-2*(sqrtdensity(counter1,1))+(sqrtdensity(counter1-1,1))))+(((sqrtdensity(counter1,1))/2)*(Vbetterguess(1,counter1+1)+((indicator)*((1/Q)*Vt*log((sqrtdensity(counter1,1)^2)/ni(1,counter1+1))))-phi));
          else
              fn(counter1,1)=((b/((x(1,counter1+1)-x(1,counter1))^2))*((sqrtdensity(counter1+1,1))-2*(sqrtdensity(counter1,1)) +(sqrtdensity(counter1-1,1))))+((sqrtdensity(counter1,1)/2)*(Vbetterguess(1,counter1+1)+((indicator)*((1/Q)*Vt*log((sqrtdensity(counter1,1)^2)/ni(1,counter1+1))))-phi));
          end
      end
      ...
      for counter1=1:No_of_nodes-2
          for counter2=1:No_of_nodes-2
              if(counter1==counter2)
                  fndash(counter1,counter2)= ((-b*2)/((x(1,counter1+1)-x(1,counter1))^2)) + ((Vbetterguess(1,counter1+1))/2) - ((phi)/2)  + ((indicator*Vt)/(2*Q))*( 2 + (log(((sqrtdensity(counter1,1)^2)/ni(1,counter1+1))) )  );                          
