function [Vg,Ctot,Gtot,Gpwtot,Cp,Gp,Gpw]=Dbt_admittance_n_fast1(HFCV,Simulation_INP)
% Global Constants
...
for jj=1:length(f)
    ...
    for ii=1:length(Vg)
        ...
        Ctnbt(ii)=Q_SI*trapz(x,Dbt1(:,ii)'./(1+0.5*imw*tau_n1(ii)*exp(2*kappa*x)));
    end
    
    Ys=imw*HFCV.Cs'+imw*Ctnbt;
