function [Vg,Ctot,Gtot,Gpwtot,Cp,Gp,Gpw,Cit,Git,Gpwit]=Dit_admittance_p1(HFCV,Simulation_INP)
% Global Constants
...
% tau=0.5et.^2)./(1+6*wt.^2);
% Gt=1e-2*ones(1,length(f));
...
for jj=1:length(f)
    ...
    Cit(jj,:)=imag(Ys1)/Omega(jj);
    Git(jj,:)=real(Ys1);
    Gpwit(jj,:)=real(Ys1)/Omega(jj);
