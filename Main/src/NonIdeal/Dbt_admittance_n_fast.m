function [Vg,Ctot,Gtot,Gpwtot,Cp,Gp,Gpw]=Dbt_admittance_n_fast(HFCV,Simulation_INP)
% Global Constants
...
%         kappa=5.107e7;
            ...
            tau_n_tmp=interp1(E,tau_n0,Efermi(ii))/(ns(ii));


        
        for xi=1:length(x)
          
            Dbt_tmp=interp1(E,Dbt(xi,:),Efermi(ii));
%             tau_n_tmp=interp1(E,tau_n0,Efermi(ii))/(ns(ii));
            Integral5=(Vt)*Dbt_tmp;
            Integral6=1/(imw*0.5*tau_n_tmp*exp(2*kappa*x(xi))+1);
            
%             Ea=E(~isnan(Integral6));
%             Integral5a=Integral5(~isnan(Integral6));
%             Integral6a=Integral6(~isnan(Integral6));
            
            Ctn_tmp(xi)=(Q_SI/Vt)*Integral5*Integral6;
        end
