function [Vg,Ctot,Gtot,Gpwtot]=BT_Yuan_Taur_n(HFCV,Simulation_INP)
% Global Constants
...
tau_n1=interp1(E,tau_n0,Efermi)./ns;
% 
% 
% for jj=1:length(f)
%     imw=1i*Omega(jj);
...
for Vgind=1:length(Vg)
    ...
    Ecbar=Ec_offset*13.6+0.74;%-Efermi(Vgind);
...
%     kappa=5.107e7;
    ...
    for jj=1:length(f)
        ...
        for ii=1:length(x)-1
            ...
            %  Y(ii+1)=(-Y(ii)*Y(ii)/(imw*eps_ox)+imw*Q_SI*Nbt(ii)/(1+imw*tau_0*exp(2*kappa*x(ii))))*(x(ii+1)-x(ii))+Y(ii);
            Y(ii+1)=(-Y(ii)*Y(ii)/(imw*eps_ox)+Q_SI*Dbt(ii,Vgind)'*log(1+imw*tau_n1(Vgind)*exp(2*kappa*x(ii)))/(tau_n1(Vgind)*exp(2*kappa*x(ii))))*(x(ii+1)-x(ii))+Y(ii); %2012 paper model
