...
function SPSCS_hf(H,OutGenerate,V_ramp)
...
% Hend=H(end,:);
H(end,:)=0;
H(end,end)=(epsilon(end)/((x(end)-x(end-1))^2))/2;
...
%         end
        ...
        while(check)
            ...
            if(OutGenerate.control.Solvers.SP)
                ...
                % killing the minority carrier response for HFCV
                % calculations
                if aa==2
            ...
            % Killing the minority carrier response completely.
            if aa==2
                if (OutGenerate.Ei(end)-OutGenerate.Ef(end))>=0
                    dn=0;
                end
